...
export default function Markseetlist() {
  ...
  const student = () => {
    axios.get("http://localhost:9002/marksheet/fetchmarksheet",{
      headers: {
  ...
  const deleteuser = async (id) => {
    await axios.delete(`http://localhost:9002/marksheet/deletemarksheet/${id}`,{
        headers: {
          "auth-token":sessionStorage.getItem("token"),
            "Content-Type": "application/json"
      }
          ...
          <tbody>
            ...
            marksheetlist.map((data, i) => 
            {
              ...
              return <tr key={i}>
                ...
                <Link className="btn btn-outline-primary m-2" to={`/editmarkshet/${data._id}`}> Edit</Link>
